...
func main() {
	...
	/*aa := spoc.PostConnections
	p, err := aa.GetByServerAndRole("10.138.32.77", "coord1")
	if err != nil {
		fmt.Println(err)
	} else {
		fmt.Println(p.ComponentRole, p.Host, p.Identity, p.Name)
	}*/
	...
	fmt.Println(con)
	str := spoc.RunSSHCommand(server, con)
	//_ = str

	var p config_prasers.Pgctl_parser
